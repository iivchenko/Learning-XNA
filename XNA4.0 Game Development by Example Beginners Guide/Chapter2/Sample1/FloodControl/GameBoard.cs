    ...
    public sealed class GameBoard
    {
        ...
        public void GenerateNewPieces(bool dropSquares)
        {
            if (dropSquares)
            {
                for (int x = 0; x < GameBoard.GameBoardWidth; x++)
                {
                    ...
                    {
                        if (GetSquare(x, y) == "Empty") { FillFromAbove(x, y); }
                    }
                }
            }
        ...
        private void UpdateFadingPieces()
        {
            ...
            foreach (string thisKey in fadingPieces.Keys)
            {
                ...
                if (fadingPieces[thisKey].alphaLevel == 0.0f) RemoveKeys.Enqueue(thisKey.ToString());
            }
            while (RemoveKeys.Count > 0) fadingPieces.Remove(RemoveKeys.Dequeue());
        }
        ...
        private void UpdateFallingPieces()
        {
            ...
            foreach (string thisKey in fallingPieces.Keys)
            {
                ...
                if (fallingPieces[thisKey].VerticalOffset == 0) RemoveKeys.Enqueue(thisKey.ToString());
            }
            while (RemoveKeys.Count > 0) fallingPieces.Remove(RemoveKeys.Dequeue());
        }
        ...
        private void UpdateRotatingPieces()
        {
            ...
            foreach (string thisKey in rotatingPieces.Keys)
            {
                ...
                if (rotatingPieces[thisKey].rotationTicksRemaining == 0) RemoveKeys.Enqueue(thisKey.ToString());
            }
            while (RemoveKeys.Count > 0) rotatingPieces.Remove(RemoveKeys.Dequeue());
        }
